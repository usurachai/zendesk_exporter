...
cleanup() {
    ...
    if vastai show instances 2>/dev/null | grep -q "^$INSTANCE_ID"; then
        ...
        vastai copy "$INSTANCE_ID" "$REMOTE_WORKSPACE/adapters/." "$LOCAL_ADAPTER_DIR/" 2>/dev/null || true
    fi
...
vastai copy "$INSTANCE_ID" "$TARBALL" /workspace/
...
if vastai copy "$INSTANCE_ID" /workspace/adapters.tar.gz "$LOCAL_ADAPTER_DIR/" 2>/dev/null; then
...
if ! $TARBALL_DOWNLOADED; then
    if vastai copy "$INSTANCE_ID" "$REMOTE_WORKSPACE/adapters/." "$LOCAL_ADAPTER_DIR/" 2>&1; then
        ...
        echo "    vastai copy $INSTANCE_ID $REMOTE_WORKSPACE/adapters/ ./adapters/"
    fi
fi
...
# --- Step 5: (Optional) Destroy instance ---
if $RENT_MODE; then
